...
/**
        ...
        public Path(Path copyPath, Edge extendBy) {
                ...
                this.distance = copyPath.distance + extendBy.weight.doubleValue(); // add the distance from extended node to original distance
                this.end = extendBy.target;
                this.dataSequence = new LinkedList<>();
                this.dataSequence.addAll(copyPath.dataSequence);
                this.dataSequence.add(end.data); // add the edge to the data sequence of copy path
        }
 ...
 /**
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if(start==null)throw new NoSuchElementException("bad start");
        if(end==null)throw new NoSuchElementException("bad end");

        // create a path based on the starting vertex
        Vertex starting = vertices.get(start);
        Vertex ending = vertices.get(end);
        // check that the vertices are valid, other wise throw NoSuchElementException
        if(starting==null)throw new NoSuchElementException("bad start");
        if(ending==null)throw new NoSuchElementException("bad end");
    
        // initialize the current path that we're looking at
        Path cp = new Path(starting);
        boolean foundPoss = false;
        //int count = 0;
    
        // create a priority queue to store the different possible paths, adding the current path
        PriorityQueue<Path> possiblePaths = new PriorityQueue();
        possiblePaths.add(cp);
    
        // set paths visited in a hashtable
        // current.end.data is key, and current(type path) is the data
        Hashtable<NodeType,Path> pathsVisited = new Hashtable();
        pathsVisited.put(cp.end.data, cp);
    
        // while the queue is not yet empty, continue to check the next shortest path
        while(!possiblePaths.isEmpty()){
                //count++;
                //if(count > 4000) throw new NoSuchElementException("none");
                // grab and remove the next shortest path in the priority queue
                cp = possiblePaths.poll();
                // if the current path reaches the end target node, we've found a legitimate path
                if(cp.end.equals(ending)) {
                        foundPoss = true;
                        break;
                }
                // compare current.end.data and current to find which path to put in pathsVisited
                if(pathsVisited.get(cp.end.data) != null && pathsVisited.get(cp.end.data).compareTo(cp) > 0) {
                        pathsVisited.put(cp.end.data, cp);
                }
    
    
                // add all vertices that come out of the ending node on the current path
                for(int i=0; i<cp.end.edgesLeaving.size(); i++) {
                        Edge myEdge = cp.end.edgesLeaving.get(i);
                        // create a new path that includes a new vertex coming out from that edge
                        Path newPath = new Path(cp,myEdge);
                        // add that new path to the priority queue if the paths visited doesn't already contain that edge's target
                        // then add the path from current to e
        ...
        // once the priority queue is emptied and we've gone through all possible full paths,
        // return whatever the shortest confirmed path is
        if(foundPoss) {
                return cp;
        }else {
                throw new NoSuchElementException("No path found bad");
        }
...
/**
     ...
     *     including when no vertex containing start or end can be found
     */ 
    public List<NodeType> shortestPathPrice(NodeType start, NodeType end) {
        return dijkstrasShortestPath(start,end).dataSequence;
    }
/**
     * Returns the shortest path between start and end.
     ...
     *     including when no vertex containing start or end can be found
     */
    public List<NodeType> shortestPathTime(NodeType start, NodeType end) {
        return dijkstrasShortestPath(start,end).dataSequence;
    }
