...
public class AE_Dijkstra<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    /**
     ...
     */
    private EdgeType getWeight(NodeType source, NodeType target) {
        if (source == null || target == null)
    ...
    protected class Path implements Comparable<Path> {
         ...
         * extended
         * by the Edge object extendBy. Use the doubleValue() method of extendBy's
         * weight field
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            List<NodeType> copyList = new LinkedList<>();
            for (NodeType n : copyPath.dataSequence) {
                copyList.add(n);
            }
            copyList.add(extendBy.target.data);
            this.dataSequence = copyList;
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if (!(vertices.containsKey(end) && vertices.containsKey(start))) {
            throw new NoSuchElementException("Invalid starting/ending vertex input");
        }
        // priority queue that stores incomplete paths
        PriorityQueue<Path> discoveredPaths = new PriorityQueue<Path>();

        // starting vertex
        Vertex initial = vertices.get(start);

        // creating a path starting with initial vertex and adding it to incomplete
        // paths queue
        Path base = new Path(initial);
        discoveredPaths.add(base);

        // visited vertices to be stored in a new hashtable as keys, with all edges
        // leaving stored as values
        Hashtable<Vertex, List<Edge>> visitedVertices = new Hashtable<>();

        // path finding
        while (!discoveredPaths.isEmpty()) {
            // removing the path at the head of the queue (current shortest path)
            Path curr = discoveredPaths.remove();

            // adding vertex and edge to visitedVertices hashtable
            visitedVertices.put(curr.end, curr.end.edgesLeaving);

            // checking for a complete path and returning it
            if (curr.end.data.equals(end)) {
                return curr;
            } else {
                // incomplete path, will loop through all edges and add all new paths to
                // priority queue using extend constructor
                for (Edge e : curr.end.edgesLeaving) {
                    if (!visitedVertices.containsKey(e.target)) {
                        discoveredPaths.add(new Path(curr, e));
                    }
                }
            }
        }
        // if no path is found, exception is thrown
        throw new NoSuchElementException("No paths found");
    }
     ...
     *                                end can be found
     */
    protected List<NodeType> shortestPath(NodeType start, NodeType end) {
     ...
     *                                end can be found
     */
    protected double getPathCost(NodeType start, NodeType end) {
        return dijkstrasShortestPath(start, end).distance;
    }

    @Override
    public String getTime(NodeType source, NodeType target) {
        return getWeight(source, target).toString();
    }

    @Override
    public int getPrice(NodeType source, NodeType target) {
        return (int) getWeight(source, target);
    }
    ...
    @Override
    public double getPathCostPrice(NodeType start, NodeType end) {
        return getPathCost(start, end);
    }

    @Override
    public double getPathCostTime(NodeType start, NodeType end) {
        return getPathCost(start, end);
    }
